...
# import sys
# reload(sys)
# sys.setdefaultencoding('gb18030')we
...
def en_to_zn_translate(content, tk):
    ...
    result = open_url(url)
    translate_result = print_result(result)
...
def translation_sys_en_to_zn(content):
    js = Return_tk()
    tk = js.getTk(content)
    t_result = en_to_zn_translate(content, tk)
    # print(t_result)
    return t_result

def translation_sys_zn_to_en(content):
    js = Return_tk()
    tk = js.getTk(content)
    t_result = zn_to_en_translate(content, tk)
    return t_result